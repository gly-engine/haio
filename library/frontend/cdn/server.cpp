...
asio::awaitable<http::response<http::vector_body<uint8_t>>> handleRequest(const Haio::Cdn::Config& config, http::request<http::string_body> req) {
    ...
    try {
        ...
        auto fetched = co_await Haio::Cdn::fetchBucket(config, route.bucket, route.path);
        if (!fetched) co_return makeError(fetched.error());
        ...
        if (!tokens.empty()) {
            ...
            if (!converted) co_return makeError(converted.error());
...
asio::awaitable<void> session(tcp::socket socket, Haio::Cdn::Config config) {
    ...
    try {
        ...
        // one bad connection must not take the server down, but dropping the reason
        // leaves nothing to debug with, so it goes to stderr and the loop carries on
